// Keeper SERVICE: direct oracle resolution and cancellation for DareU v3.
//
...
// settle_market_action transaction (RESOLVE or CANCEL). There is no proposal, bond,
// challenge, dispute, vote, or finalization phase.
import { Outcome, SettlementAction } from '../../src/managed/dareu-v3/contract/index.js'
import {
  loadEnvFiles,
  parseHexBytes,
  pgExec,
...
export async function resolveMarketsV3(
  ...
  if (rows.length === 0) {
    console.log('[resolve-v3] no ready_to_resolve markets.')
  ...
  try {
    for (const row of rows as Array<{ id: string; outcome: 'yes' | 'no' }>) {
      try {
        ...
        abortBatchIfWalletUnavailable(`resolve-v3 ${row.id.slice(0, 12)}`, error)
      }
    }
  } finally {
...
export async function cancelRequestedV3(
  ...
  if (rows.length === 0) {
    console.log(`[cancel-v3:${mode}] no cancel_requested markets.`)
    return { selected: 0, succeeded: 0 }
  }
  ...
  try {
    for (const row of rows as Array<{ id: string }>) {
      try {
        ...
        abortBatchIfWalletUnavailable(`cancel-v3 ${row.id.slice(0, 12)}`, error)
      }
    }
  } finally {
...
async function main() {
  ...
  await resolveMarketsV3(network)
  await cancelRequestedV3(network)
}
