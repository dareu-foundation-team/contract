/**
 * Persist a V3 ledger snapshot without rewriting every market on every sync.
 *
 ...
 */
export const V3_MARKET_MIRROR_UPDATE_SQL = `WITH incoming AS (
   SELECT *
     FROM unnest(
 ...
 UPDATE markets AS market
    SET status = CASE
          WHEN state.status IN ('resolved', 'cancelled') THEN state.status
          ELSE market.status
        END,
        outcome = CASE
          WHEN state.status = 'resolved' THEN state.outcome
          WHEN state.status = 'cancelled' THEN NULL
          ELSE market.outcome
        END,
        onchain_status = state.status,
        ...
        updated_at = CASE
          WHEN state.status IN ('resolved', 'cancelled')
            AND market.status IS DISTINCT FROM state.status THEN now()
          ELSE market.updated_at
        END
   FROM incoming AS state
  WHERE market.id = state.id
    AND market.onchain_contract_version = 'v3'
    AND market.onchain_contract_address = $6
    AND (
      ...
      market.onchain_contract_version IS DISTINCT FROM 'v3' OR
      market.onchain_contract_address IS DISTINCT FROM $6 OR
      (
        state.status IN ('resolved', 'cancelled') AND
        market.status IS DISTINCT FROM state.status
      ) OR
