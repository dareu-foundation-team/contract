...
export async function publishDrafts(network: ReturnType<typeof resolveNetwork>) {
  ...
  let ok = 0
  try {
    for (const row of rows as Array<{ id: string; metadata_hash: string; oracle_participant_id: string; close_unix: string }>) {
      try {
        ...
        await pgExec(
          dbUrl,
          `UPDATE markets SET status='open', onchain_tx_id=$2, updated_at=now() WHERE id=$1`,
          [row.id, txId || 'onchain'],
        )
        ok++
